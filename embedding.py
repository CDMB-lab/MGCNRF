...
def cross_validation_experiment(drug_dis_matrix, drug_matrix, dis_matrix, seed, epochs, emb_dim, dp, lr, adjdp):
    ...
    # wbk = xlsxwriter.Workbook('E:/桌面/feature.xlsx')
    # sheet = wbk.add_worksheet('sheet1')
    # for i in range(drug_len):
    #     for j in range(dis_len):
    #         sheet.write(i, j, predict_y_proba.reshape[i][j])
    #         wbk.close()
...
if __name__ == "__main__":
    drug_sim = np.loadtxt('../data/m_m_s.csv', delimiter=',')
    dis_sim = np.loadtxt('../data/d_d_s.csv', delimiter=',')
    drug_dis_matrix = np.loadtxt('../data/m_d.csv', delimiter=',')
